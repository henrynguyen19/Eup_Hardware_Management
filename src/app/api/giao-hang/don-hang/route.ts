...
const VALID_STATUSES = ['cho_xu_ly', 'dang_xu_ly', 'da_gui', 'hoan_thanh', 'da_huy']

// ─── GET /api/giao-hang/don-hang?mine=1&status=&page=1 ───────────────────────
export async function GET(req: NextRequest) {
  ...
  const mine   = sp.get('mine') !== '0'       // default: show own orders
  const status = sp.get('status') ?? ''
  const page   = parseInt(sp.get('page') ?? '1')
  const limit  = Math.min(parseInt(sp.get('limit') ?? '50'), 100)
  const offset = (page - 1) * limit

  // Fetch permission to see if admin
  ...
  // If not admin, always filter to own orders
