      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="flex gap-0">
            {/* Image */}
            <div className="w-64 flex-shrink-0 bg-gradient-to-br from-gray-50 to-gray-100 flex items-center justify-center p-4 min-h-64">
              {card.main_photo ? (
                <div className="relative w-56 h-56">
                  <Image src={card.main_photo} alt={card.name} fill className="object-contain" />
                </div>
              ) : (
                <div className="text-8xl opacity-20">{DEVICE_TYPE_ICONS[dt]}</div>
              )}
            </div>
