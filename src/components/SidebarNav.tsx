...
export default function SidebarNav({ userEmail, isAdmin, canHoTro, canChatLuong = false, canSuaChua = false, canKho = false, canChungNhan = false, canKhoDaily = false }: Props) {
  ...
  const navItems: NavItem[] = [
    ...
    { icon: '🚚', label: t.sidebar.delivery,    href: '/giao-nhan',    show: true },
  ...
  return (
    <>
      ...
      {/* ── SIDEBAR ───────────────────────────────────── */}
      {/* Desktop: always in-flow (hidden md:flex pattern).
          Mobile: hidden by default, shows as fixed overlay when mobileOpen=true */}
      <aside
        className={
          mobileOpen
            ? 'fixed inset-y-0 left-0 z-50 flex flex-col w-56 min-h-screen'   /* mobile open: overlay */
            : 'hidden md:flex flex-col flex-shrink-0 w-56 min-h-screen'         /* mobile closed + desktop */
        }
